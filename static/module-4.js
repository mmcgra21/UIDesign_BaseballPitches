$(document).ready(function() {
    function display_flashcard(i) {
        ...
        btn.append(img);
        $(btn).click(function(){
            window.location.href = '/flashcard-hard-'+pitches[i].id;
        });
    ...
    for (let i = 0; i < pitches.length; i++) {
        ...
        if (pitches[i].flashcard_hard_completed) {
    ...
    let completed = true;
    for (let i = 0; i < pitches.length; i++) {
        started ||= pitches[i].flashcard_hard_completed;
        completed &&= pitches[i].flashcard_hard_completed;
    }
    if (started) {
