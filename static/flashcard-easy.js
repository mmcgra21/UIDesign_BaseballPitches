$(document).ready(function() {
    ...
    col.append($("<div class='img-row row'><img class='flashcard-pitch-img' src='/static/images/pitch_diagrams/diagram-"+pitches[pitchId-1].id+".png'></div>"));
    col.append($("<div class='done-btn-row row'><button type='submit' id='done-btn'>Done</button></div>"));
    $(".flashcard-page").append(col);
    // Mark current pitch's easy flashcard as completed
    if (!pitches[pitchId-1].flashcard_easy_completed) {
        $.ajax({
            type: "POST",
            url: "update",
            async: false,
            ...
            data: JSON.stringify(
                {
                    "id":pitchId,
                    "variable":"pitches",
                    "field":"flashcard_easy_completed",
                    "value":true
                }
            ),
    ...
    if (!modules[0].completed) {
        ...
        for (let i = 0; i < pitches.length; i++) {
            isComplete &&= pitches[i].flashcard_easy_completed;
        }
        if (isComplete) {
            $.ajax({
                type: "POST",
